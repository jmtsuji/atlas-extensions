...
if [ $# == 0 ]
    ...
    printf "   6. mapping_guide_file: MUST be specified if you run in 'normal' mode but is not needed for 'coassembly' mode. A tab-separated file (with headers). Column 1 (sample_name) should be the ID of the metagenome you want to map onto the ATLAS assembly -- name can be whatever you like but CANNOT start with numbers!. Column 2 (raw_read_filepath) should be a comma-separated list of complete filepaths to the raw read files (e.g., R1, R2, se) for that metagenome - no spaces. You must supply either R1,R2 or R1,R2,se for the script to work.\n\n"
